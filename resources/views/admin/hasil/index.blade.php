...
@endif
    ...
    <div class="p-4 bg-white rounded-lg">
        <div class="inline-flex rounded-md shadow-sm mb-3" role="group">
            <!-- Modal toggle -->
            <a href="{{ route('hasil.create') }}" class="inline-flex items-center text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h2 class="text-2xl font-bold mb-4">Data Cacah Hasil Kuisioner</h2>
            <table id="example" class="table-auto w-full">
                <thead>
                    <tr>
                        ...
                        <th class="px-4 py-2">Bobot Harapan</th>
                        <th class="px-4 py-2">Bobot Persepsi</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="border px-4 py-2">{{ $data->responden->name }}</td>
                        <td class="border px-4 py-2">{{ $data->pertanyaan->kode_pertanyaan }}</td>
                        <td class="border px-4 py-2">{{ $data->bobot_harapan }}</td>
                        <td class="border px-4 py-2">{{ $data->bobot_persepsi }}</td>
                        <td class="border px-4 py-2">
                            <div class="inline-flex justify-center rounded-md shadow-sm" role="group">
                                ...
                                action="{{ route('hasil.destroy', $data->id) }}" method="POST"
                                    style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
